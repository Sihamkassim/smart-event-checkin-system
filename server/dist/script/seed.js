...
async function seedDatabase() {
    ...
    try {
        ...
        const admin = await index_js_1.User.create({
            name: 'Admin User',
            email: 'admin@lahn.test',
            password_hash: hashedPassword,
            role: 'admin',
        });
        const staff = await index_js_1.User.create({
            name: 'Staff User',
        ...
        // ============================================
        const visitors = await index_js_1.Visitor.bulkCreate([
            {
                event_id: event1.id,
                full_name: 'John Doe',
                phone: '+1234567890',
        ...
        // ============================================
        const checkInLogs = await index_js_1.CheckInLog.bulkCreate([
            {
                event_id: event1.id,
        ...
        console.log('========================================');
        console.log('📊 Summary:');
        console.log(`  - ${await index_js_1.User.count()} users`);
        console.log(`  - ${await index_js_1.Event.count()} events`);
        console.log(`  - ${await index_js_1.Visitor.count()} visitors`);
        console.log(`  - ${await index_js_1.CheckInLog.count()} check-in logs`);
        console.log('========================================');
        console.log('\n🔐 Login Credentials:');
